...
Here are some things you might explore next:

- Add a new chapter with `chapter-2.md` and include it in `publish.txt`
- Drop a new image in `assets/` and reference it in your content
- Try editing the `book.yaml` in `.pandoc/metadata/` to adjust margins, fonts, or header styling
- Create a second output format (`make publish format=epub`) and try reading it on an e-reader
