...
# Prefer user-provided metadata file because it can handle unicode
if [[ -f .pandoc/metadata/user-metadata.yaml ]]; then
  PANDOC_OPTS+=(--metadata-file=user-metadata.yaml)
else
  if [[ "$KEYSTONE_LANG" != "" && "$KEYSTONE_LANG" != "en-US" ]]; then
    echo "⚠️  Warning: Non-default locale '\$KEYSTONE_LANG' may cause issues with Pandoc CLI when injecting metadata."
    echo "   Consider using a user-provided metadata file (user-metadata.yaml) instead."
  fi
  PANDOC_OPTS+=(
    -M title="$KEYSTONE_TITLE"
    -M subtitle="$KEYSTONE_SUBTITLE"
    -M author="$KEYSTONE_AUTHOR"
    -M lang="${KEYSTONE_LANG:-en-US}"
