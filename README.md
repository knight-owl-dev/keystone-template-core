...
**2. Configure your project:** `project.conf` sets the project name and Docker naming. `pandoc.yaml` holds all book metadata — title, author, description, layout, and cover image. Both files are version-controlled and used at build time.
...
- **`pandoc.yaml`** — all book metadata: title, author, description, keywords,
...
Only `chapters/`, `appendix/`, and `assets/` are recognized by the
