.PHONY: image import publish all clean sample help
...
sample:
	@if grep -Ev '^\s*#|^\s*$$' publish.txt | grep -q .; then \
		echo "ERROR: Cannot install sample content: publish.txt is not empty." >&2; \
		echo "   Please clear it manually or move your content before running 'make sample'." >&2; \
		exit 1; \
	fi
	@echo "Installing Keystone sample content..."
	@rsync -av .keystone/sample/ ./
	@echo "OK: Sample content installed. You can now run 'make all' to build your first book."
	@echo ""
